	...
	File.open("#{slug}.md", 'w') do |file|
		...
		image_parsed["results"].each do |image|
			...
			open("#{image["url_fullxfull"]}") do |image|
			  File.open("./#{listing["listing_id"]}_full_#{count}.jpg", "wb") do |file|
			    file.write(image.read)
			  end
			end

			open("#{image["url_570xN"]}") do |image|
			  File.open("./#{listing["listing_id"]}_medium_#{count}.jpg", "wb") do |file|
			    file.write(image.read)
			  end
			end

			open("#{image["url_75x75"]}") do |image|
			  File.open("./#{listing["listing_id"]}_thumbnail_#{count}.jpg", "wb") do |file|
			    file.write(image.read)
			  end
			end
